...
module.exports = app => {
    ...
    app.runSchedule = (...args) => {
        if (decorator_1.isClass(args[0])) {
            args[0] = decorator_1.getProviderId(args[0]) + '#' + args[0].name;
        }
        return originMethod.call(app, ...args);
    };
    const schedules = decorator_1.listModule(decorator_1.SCHEDULE_KEY);
    for (const scheduleModule of schedules) {
        const providerId = decorator_1.getProviderId(scheduleModule);
        if (providerId) {
            const key = providerId + '#' + scheduleModule.name;
            const opts = decorator_1.getClassMetadata(decorator_1.SCHEDULE_KEY, scheduleModule);
